export const People = () => {
  return (
    <div>
      <p className="text-center text-3xl font-bold">Vezetőink</p>
      <div className="m-5 grid auto-cols-auto gap-4">
        <div className=" bg-[#121212] p-3">
          <p>Kovács Gyula</p>
          <p>vezérigazgató</p>
          <a>+36 20 374 0014</a>
          <a>volvid.kovacsgy@volvid.hu</a>
        </div>
        <div className=" bg-[#121212] p-3">
          <p>Botos Kornél</p>
          <p>műszaki igazgató</p>
          <a>+36 20 479 6353</a>
          <a>volvid.botosk@volvid.hu</a>
        </div>
        <div className=" bg-[#121212] p-3">
          <p>Kuruczné Prescher Viktória</p>
          <p>termelési igazgató</p>
          <a>+36 20 403 5353</a>
          <a>volvid.prescherv@volvid.hu</a>
        </div>
        <div className=" bg-[#121212] p-3">
          <p>Tarnóczi Péter</p>
          <p>termelési igazgató</p>
          <a>+36 20 403 5353</a>
          <a>volvid.prescherv@volvid.hu</a>
        </div>
        <div className=" bg-[#121212] p-3">
          <p>Nagy László</p>
          <p>termelési igazgató</p>
          <a>+36 20 403 5353</a>
          <a>volvid.prescherv@volvid.hu</a>
        </div>
        <div className=" bg-[#121212] p-3">
          <p>Polestyuk Zoltán</p>
          <p>termelési igazgató</p>
          <a>+36 20 403 5353</a>
          <a>volvid.prescherv@volvid.hu</a>
        </div>
        <div className=" bg-[#121212] p-3">
          <p>Maár Viktória</p>
          <p>termelési igazgató</p>
          <a>+36 20 403 5353</a>
          <a>volvid.prescherv@volvid.hu</a>
        </div>
        <div className=" bg-[#121212] p-3">
          <p>Zsigmond Hajnalka</p>
          <p>termelési igazgató</p>
          <a>+36 20 403 5353</a>
          <a>volvid.prescherv@volvid.hu</a>
        </div>
      </div>
    </div>
  );
};
